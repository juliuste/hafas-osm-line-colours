	...
	if (ref && colour && products) {
		for (let location of locations) {
			...
			for (let product of products) {
				result.push([lon, lat, { ref, colour, product }])
			}
		}
	}
...
const createLookup = (lines) => {
	const points = unionBy(...lines.map(lineToPoints), x => JSON.stringify(x))
	...
	const lookup = (location, product) => {
		...
		return uniqBy(results, x => `${x.ref}|${x.colour}`)
	}
	return lookup
}
